...
def train():
    ...
    if training_args.fix_llm:
        ...
        if model_args.mae_fp != 2:
            model.mae_predict_head.requires_grad_(True)
            model.get_model().mask_token.requires_grad_(True)
        if model_args.recon_fp != 2:
            model.recon_predict_head.requires_grad_(True)
        if model_args.pos_embed_mae != 0:
            model.get_model().decoder_pos_embed.requires_grad_(True)
    else:
