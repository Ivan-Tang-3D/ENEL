...
from pointllm.eval.gpt_eval import gpt_get_average_score
...
def main(args):
    ...
    # print(model)
    openaigpt = OpenAIGPT(**default_chat_parameters)
    ...
    with tqdm(total=len(ans_lines), desc="Processing tasks", unit="task") as pbar:
        def process_task(i):
            ...
            score = gpt_get_average_score(question, category, model_ans, gt_ans, openaigpt, model=model_name, times=args.times)
            score = round(score, 1)
